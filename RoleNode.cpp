...
void RoleNode::moveToward(const Point toTileCoord)
{
    ...
    if(!isHasBarrier)//起点终点两点之间无障碍，则不采用A＊避免在不必要的时候依然使用A*寻路
    {
        ...
        MoveTo *moveAction = MoveTo::create(1.5f*time, _layer->positionForTileCoord(toTileCoord));
...
void RoleNode::getBorderUponPlace()
{
    ...
    int time = sqrt(distanceX*distanceX+distanceY*distanceY);
    MoveTo *moveAction = MoveTo::create(1.5f*time, _layer->positionForTileCoord(endPosition));
    moveAction->setTag(1);
    this->runAction(moveAction);
}
//处理三点共线
        ...
        if (len - iStarIndex > 3)
        {
            ...
            for (ssize_t i = len - 1; i > iStarIndex; i--)
            {
                ...
                if(!isHasBarrier)//两点之间无障碍，则合并
                {
                    ...
                    {
                        CCLOG("delete point %f,%f", _shortestPath.at(ia)->getPosition().x,_shortestPath.at(ia)->getPosition().y);
...
void RoleNode::searchInRoadObject(cocos2d::Point toTileCoord)
{
    ...
    while (1)
    {
        ...
        for (ssize_t i = 0; i < adjSteps->count(); ++i)
        {
            ...
            //CCLOG(" runIndex %d   step: %f, %f",runIndex, step->getPosition().x, step->getPosition().y);
        }
...
void RoleNode::popStepAndAnimate()
{
    ...
    //组合动作和回调
    MoveTo *moveAction = MoveTo::create(1.5f*time, _layer->positionForTileCoord(step->getPosition()));
